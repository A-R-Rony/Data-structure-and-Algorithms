...
unsigned long long int a, b, MOD;
struct Matrix
{
    ll n, m;
    vector<vector<ull>>v;
    Matrix() {}
    Matrix(ll _n, ll _m) {n = _n; m = _m; v.assign(n, vector<ull>(m, 0));}
    Matrix(vector<vector<ull>>a) {n = a.size(); m = n ? a[0].size() : 0; v = a;}
...
void solve()
{
    ...
    Matrix an = a1.expo(b - 1);
    ull rs = (an.v[0][0] * a) % MOD + (an.v[0][1] * 2) % MOD;
    rs %= MOD;
    print(rs);
}
int main()
...
// https://codeforces.com/gym/104619/problem/E
